...
- [SoftwareUpdateSettingsBeta_ProgramObject](/documentation/devicemanagement/softwareupdatesettingsbeta_programobject) - The name and token associated with a specific beta program to be allowed.
- [SoftwareUpdateSettingsBeta_RequireProgramObject](/documentation/devicemanagement/softwareupdatesettingsbeta_requireprogramobject) - The device automatically enrolls in this beta program. This key must only be present if the `ProgramEnrollment` key is set to `AlwaysOn`. The `OfferPrograms` key must not be present if this key is present.
