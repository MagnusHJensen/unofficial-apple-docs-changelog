...
- ****: Allows devices and device management services to advertise the changes in their capabilities over time.
...
- **** — The valid and active states for all declarations on the device, and any error details describing why a declaration is invalid or inactive.
- **** — Properties of the device (for example, device model, OS version, and so on).
- **** — Including accounts, passcode, and apps installed using device management.
...
Status item arrays have values containing JSON objects with a well-defined schema, with the same schema used for all items in an array. The schemas are specific to the type of item the status represents. Devices report changes to array status items incrementally (see [StatusReport](/documentation/devicemanagement/statusreport) for additional details).
...
When the status of a device changes or a status subscription declaration becomes active, the device sends a [StatusReport](/documentation/devicemanagement/statusreport) to the device management service.
...
The device advertises the set of declaration types and status items it supports with the [StatusManagementClientCapabilities](/documentation/devicemanagement/statusmanagementclientcapabilities) status item. When the device enables declarative management, the device sends this status item to the device management service. Device management services need to persist this on a per-enrollment basis as the set of capabilities might vary by more than just device type or operating system version. The service shouldn’t send declarations to the device that it hasn’t advertised as supported.

The device management service advertises the set of protocol features it supports through the [ManagementServerCapabilities](/documentation/devicemanagement/managementservercapabilities) declaration that it sends to the device when it enables declarative management. If the service capabilities change any time after enablement, then the service should use the normal declaration synchronization process to update the device with the new values.
