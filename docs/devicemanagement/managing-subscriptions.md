...
When your MDM client disassociates a user from a ** seat, that seat returns to the available pool as a renewing seat. The organization retains the seat, and your MDM client can create an assignment for another user. The total number of seats the organization owns remains unchanged.
...
Expose the ** `expiring` counts in your MDM client so administrators can monitor capacity that they’ll lose at the end of the billing period. Administrators can then choose to create expiring assignments from those seats before the period ends, or purchase additional seats to compensate.
